	...
	class Cate
	{
		...
		*/
		...
		public function actionEdit($data)
		{
			...
			{
				$category['password'] = testInputString($data['password']);
			}
			else
			{
				$error[] ="password";
			...
			{
				# code...
				$this->db->update('user',$category,array("id" => $id));
			...
			{
				$_SESSION['error'] = "Bạn cần nhập đầy đủ các trường có dấu (*).";
				rdr_url("../../admin/tai-khoan/index.php?action=edit&{$id}");
			}
		}
		...
		public function delete($data)
		{
			if (isset($_GET['id'])) {
				...
				$_SESSION['error'] = "Tai khoan không tồn tại";
				rdr_url("../../admin/tai-khoan/index.php");	
			}
		...
		{
			$data_cate = $this->db->fetchwhere("user","id = ".$id );

			if( empty($data_cate) ) {
				$_SESSION['error'] = "Không tồn tại tai khoan";
				rdr_url("../../admin/tai-khoan/index.php");
			}
			
			$this->db->delete('user',$id);
			$_SESSION['success'] = "Tai khoan đã được xóa.";
			rdr_url("../../admin/tai-khoan/index.php");	
